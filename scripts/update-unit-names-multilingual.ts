...
async function main() {
  console.log('🚀 Starting Multilingual Unit Name update...')
  ...
  for (const unit of units) {
    ...
    if (unit.ref_areas) {
      ...
      // Determine if it's a CA unit based on the logic from previous script
      // (194th unit onwards was mapped to CA which is Мирабадский район ID 977)
      // Actually, let's check if it's linked to the CA military district (ID 4)
      ...
      if (unit.military_district_id === 4 && area.id === 977 && unit.unit_id > 193) {
        ...
        const areaUz = areaName.uz || 'Tuman'
        const areaUzk = areaName.uzk || 'Туман'
  ...
  console.log(`\n🎉 Multilingual update complete! Updated ${updatedCount} units.`)
}
