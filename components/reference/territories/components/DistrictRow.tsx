...
import { Edit, Trash2, MapPin, Building2, MoreHorizontal, CheckCircle2, XCircle, LocateFixed } from "lucide-react"
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
import { LocalizedContent, District } from "../types"
...
export function DistrictRow({ item, idx, onEdit, onDelete, getLocalizedName, getSubtextNames, t, ui }: DistrictRowProps) {
    return (
        <TableRow className="group h-20 hover:bg-blue-50/50 transition-all duration-300 border-b border-border/40">
            ...
            <TableCell className="px-6 border-l border-border/5">
                <div className="flex items-center gap-3">
                    <div className="p-2 rounded-xl bg-indigo-100/50 text-indigo-600 shadow-sm group-hover:bg-indigo-600 group-hover:text-white transition-all scale-90 group-hover:scale-100 group-hover:shadow-lg group-hover:shadow-indigo-500/20">
                        {item.type === "City" ? <Building2 className="h-4 w-4" /> : <LocateFixed className="h-4 w-4" />}
                    </div>
                    <div className="flex flex-col">
                        <span className="font-bold text-[14px] text-slate-900 group-hover:text-indigo-700 transition-colors">
                            {getLocalizedName(item.name)}
                        </span>
                        <span className="text-[10px] font-medium text-muted-foreground/60 w-48 truncate">
            ...
            <TableCell className="px-6 border-l border-border/5">
                <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[10px] text-muted-foreground/60 pl-4.5 truncate max-w-[150px]">
                        {getSubtextNames(item.region)}
                    </span>
                </div>
            </TableCell>
            <TableCell className="px-6 border-l border-border/5">
