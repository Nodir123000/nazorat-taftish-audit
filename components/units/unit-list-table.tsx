...
export function UnitListTable({ data, isLoading, onEdit, onDelete }: UnitListTableProps) {
    ...
    return (
        <div className="border-none shadow-2xl shadow-slate-200/50 bg-white/80 backdrop-blur-xl rounded-[32px] overflow-hidden">
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="hover:bg-transparent border-b border-slate-100 h-20 bg-slate-50/50 flex items-center">
                            <TableHead className="w-[60px] px-4 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center justify-center flex shrink-0">ID</TableHead>
                            <TableHead className="w-[90px] px-2 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center justify-center flex shrink-0">{t("Штат ID", "Shtat ID", "Штат ID")}</TableHead>
                            <TableHead className="w-[400px] px-6 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center flex shrink-0">{t("Наименование", "Nomlanishi", "Номи")}</TableHead>
                            <TableHead className="flex-1 px-6 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center flex shrink-0">{t("Локация", "Joylashuvi", "Локация")}</TableHead>
                            <TableHead className="w-[320px] px-6 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center flex shrink-0">{t("Округ", "Okrug", "Округ")}</TableHead>
                            <TableHead className="w-[100px] px-4 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center justify-center flex shrink-0">{t("Статус", "Holati", "Ҳолати")}</TableHead>
                            <TableHead className="w-[120px] px-8 font-black text-[10px] uppercase tracking-widest text-slate-400 items-center justify-end flex shrink-0">{t("Действия", "Harakatlar", "Ҳаракатлар")}</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {isLoading ? (
                            ...
                            data.map((unit) => (
                                <TableRow
                                    ...
                                    style={{ height: '96px' }}
                                >
                                    <TableCell className="w-[60px] px-4 font-mono text-[10px] font-bold text-slate-400 justify-center flex shrink-0">
                                        {unit.unitId}
                                    </TableCell>
                                    <TableCell className="w-[90px] px-2 font-mono text-xs font-black text-slate-700 justify-center flex shrink-0">
                                        {unit.unitCode || unit.stateId || "-"}
                                    </TableCell>
                                    <TableCell className="w-[400px] px-6 flex shrink-0 items-center gap-4 overflow-hidden">
                                        ...
                                        <div className="flex flex-col min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                <Badge variant="outline" className="rounded-md px-1.5 py-0 text-[9px] uppercase font-black bg-white text-slate-400 border-slate-200 tracking-tighter">
                                                    {unit.unitType || unit.type || t("Часть", "Qism", "Қисм")}
                                                </Badge>
                                                {unit.unitCode && (
                                                    <span className="text-[9px] font-mono font-bold text-slate-300 bg-slate-50/50 px-1.5 py-0.5 rounded-md border border-slate-100">
                                                        {unit.unitCode}
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    <TableCell className="flex-1 px-6 text-xs text-slate-600 font-medium flex shrink-0 flex-col justify-center gap-1.5 min-w-0">
                                        ...
                                        <div className="flex items-center gap-1 pl-5">
                                            ...
                                            <span className="text-[11px] text-slate-400 truncate tracking-tight">{getLocalizedName(unit.area?.region) || "-"}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="w-[320px] px-6 flex shrink-0 items-center">
                                        {unit.district ? (
                                            <div className="bg-slate-50 border border-slate-100/50 rounded-xl px-4 py-2 w-full flex items-center gap-3 group-hover:bg-white transition-colors shadow-sm">
                                                ...
                                                <span className="text-[10px] font-black uppercase tracking-tight text-slate-600 leading-none">
                                    ...
                                    <TableCell className="w-[100px] px-4 justify-center flex shrink-0">
                                        <Badge
                                            variant="outline"
                                            className={cn(
                                                "rounded-lg px-2.5 py-1.5 font-black text-[9px] uppercase tracking-widest border whitespace-nowrap shadow-sm",
                                    ...
                                    <TableCell className="w-[120px] px-4 text-right shrink-0 flex justify-end gap-1.5">
                                        <Button
                                            variant="ghost"
                                            size="icon"
